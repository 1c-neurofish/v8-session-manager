...
- Статус: accepted (частично пересмотрен 2026-05-09 — см. ниже)
- Дата: 2026-05-06
- Контекст задачи: extraction (см. `tasks/session-manager-extraction/`)

> **Уточнение от 2026-05-09 (без отдельного ADR):** утверждение ниже о
> публикации проксированных тулов как `<prefix>__<tool>` (раздел «Решение»,
> а также «Положительные» и «Проверка» в «Последствиях») **снято**. Префикс
> убран: `published_name == tool_name`, дедупликация ведётся только по
> `(tool_name, schema_hash)`, `kind` информационный. Регистрация дополнительно
> требует обязательных полей `infobase_name` и `ib_session_number`. Подробности
> и SessionListItem v2 — в `README.md`, секция «Naming contract (v2)».
> ADR-0025 в части префикса `<kind>__<tool_name>` тоже устарел — см. ту же
> секцию README.
